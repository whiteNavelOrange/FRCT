...
supported_agents = {
    ...
    "bimanual": ("BIMANUAL_PERACT", "ACT_BC_LANG", "relative_act", "RRVT"),
    "unimanual": (),
}
...
def create_agent(cfg: DictConfig) -> Agent:
    ...
    elif agent_type == "bimanual" or agent_type == "unimanual" or agent_type == "relative":
        return agent_fn(cfg)
    else:
...
def agent_fn_by_name(method_name: str) -> Agent:
    ...
    elif method_name.startswith("relative"):
        from agents import relative_act

        return relative_act.launch_utils.create_agent
    elif method_name.startswith("RRVT"):
        from agents import rrvt

        return rrvt.launch_utils.create_agent
