...
from rvt.models import rrvt_agent
from rvt.mvt.rmvt import RMVT
...
def create_agent(cfg: DictConfig):
    ...
    agent = RVTAgentWrapper(
        cfg.framework.checkpoint_name_prefix, cfg.rlbench, mvt_cfg, exp_cfg, cfg.framework.training_iterations
    )
...
class RVTAgentWrapper(Agent):
    def __init__(self, checkpoint_name_prefix, rlbench_cfg, mvt_cfg, exp_cfg, training_iterations):
    ...
    def build(self, training: bool, device=None) -> None:
        ...
        self.rvt_agent = rrvt_agent.RRVTAgent(
            network=rvt,
            image_resolution=self.rlbench_cfg.camera_resolution,
            rot_ver=self.mvt_cfg.rot_ver,
            ...
            cos_dec_max_step=self.training_iterations,
            **self.exp_cfg.peract,
            **self.exp_cfg.rvt,
        )
