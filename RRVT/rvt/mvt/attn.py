...
class SpatialSoftmax2D(nn.Module):
    def __init__(self, height, width, channel):
        ...
        pos_x = torch.from_numpy(
            pos_x.reshape(self.height * self.width)
        ).float()
        pos_y = torch.from_numpy(
            pos_y.reshape(self.height * self.width)
        ).float()
        self.register_buffer("pos_x", pos_x)
        self.register_buffer("pos_y", pos_y)
...
class SpatialSoftmax1D(nn.Module):
    def __init__(self, channel, length):
        super(SpatialSoftmax1D, self).__init__()

        self.length = length
        self.channel = channel
        pos = np.linspace(-1, 1, length)
        pos = torch.from_numpy(pos).float()
        self.register_buffer('pos', pos)
        self.temperature = 0.01

    def forward(self, feature):
        feature = feature.view(-1, self.length)
        attention = F.softmax(feature / self.temperature, dim=-1)
        expected = torch.sum(self.pos * attention, dim=1, keepdim=True)
        feature_max = expected.view(-1, 1 * self.channel)
        return feature_max