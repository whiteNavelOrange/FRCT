...
def perturb_se32(pcd, tran_rot_shift_4x4, bounds):
    """Perturb point clouds with given transformation.
    :param pcd:
        Either:
        - list of point clouds [[bs, 3, H, W], ...] for N cameras
    ...
    for p in pcd:
        ...
        # apply rotation
        perturbed_p_flat_4x1_action_origin = torch.bmm(
            p_flat_4x1_action_origin.transpose(2, 1), tran_rot_shift_4x4
        ).transpose(2, 1)
        ...
        # shift back the origin
        perturbed_p_flat_3x1 = (
            perturbed_p_flat_4x1_action_origin[:, :3, :] 
        )
        if permute_p:
            perturbed_p_flat_3x1 = torch.permute(perturbed_p_flat_3x1, (0, 2, 1))
        perturbed_p = perturbed_p_flat_3x1.reshape(p_shape)
...
def apply_se3_aug_con(
    ...
    # trans_shift = torch.tensor([[0.1, 0.1, 0.1]])
    trans_shift_4x4 = identity_4x4.detach().clone()
    trans_shift_4x4[:, 0:3, 3] = trans_shift
...
def apply_se3_aug_con_relative(
    pcd,
    ...
    # batch size
    bs = right_action_gripper_pose.shape[0]
    device = right_action_gripper_pose.device
    ...
    if ver == 1:
        right_action_gripper_quat_wxyz = torch.cat(
            (right_action_gripper_pose[:, 6].unsqueeze(1), right_action_gripper_pose[:, 3:6]), dim=1
        )
        right_action_gripper_rot = torch3d_tf.quaternion_to_matrix(right_action_gripper_quat_wxyz)
        left_action_gripper_quat_wxyz = torch.cat(
            (left_action_gripper_pose[:, 6].unsqueeze(1), left_action_gripper_pose[:, 3:6]), dim=1
        )
        left_action_gripper_rot = torch3d_tf.quaternion_to_matrix(left_action_gripper_quat_wxyz)

    elif ver == 2:
        # applying gimble fix to calculate a new action_gripper_rot
        r = Rotation.from_quat(right_action_gripper_pose[:, 3:7].cpu().numpy())
        euler = r.as_euler("xyz", degrees=True)
        euler = aug_utils.sensitive_gimble_fix(euler)
        right_action_gripper_rot = torch.tensor(
            Rotation.from_euler("xyz", euler, degrees=True).as_matrix(),
            device=device,
        )
        r = Rotation.from_quat(left_action_gripper_pose[:, 3:7].cpu().numpy())
        euler = r.as_euler("xyz", degrees=True)
        euler = aug_utils.sensitive_gimble_fix(euler)
        left_action_gripper_rot = torch.tensor(
            Rotation.from_euler("xyz", euler, degrees=True).as_matrix(),
            device=device,
        )
    else:
    ...
    if ver == 1:
        ...
        left_action_gripper_4x4[:, 0:3, 3] = left_action_gripper_trans - right_action_gripper_trans
    # sample translation perturbation with specified range
    # augmentation range is a percentage of the scene bound
    trans_range = (bounds[:, 3:] - bounds[:, :3]) * trans_aug_range.to(device=device)
    # rand_dist samples value from -1 to 1
    ...
    trans_shift[:, 0] = torch.clamp(
        trans_shift[:, 0],
        min=bounds_x_min - right_action_gripper_trans[:, 0],
        max=bounds_x_max - right_action_gripper_trans[:, 0],
    )
    trans_shift[:, 1] = torch.clamp(
        trans_shift[:, 1],
        min=bounds_y_min - right_action_gripper_trans[:, 1],
        max=bounds_y_max - right_action_gripper_trans[:, 1],
    )
    trans_shift[:, 2] = torch.clamp(
        trans_shift[:, 2],
        min=bounds_z_min - right_action_gripper_trans[:, 2],
        max=bounds_z_max - right_action_gripper_trans[:, 2],
    )
    trans_shift[:, 0] = torch.clamp(
        trans_shift[:, 0],
        min=bounds_x_min - left_action_gripper_trans[:, 0],
        max=bounds_x_max - left_action_gripper_trans[:, 0],
    )
    trans_shift[:, 1] = torch.clamp(
        trans_shift[:, 1],
        min=bounds_y_min - left_action_gripper_trans[:, 1],
        max=bounds_y_max - left_action_gripper_trans[:, 1],
    )
    trans_shift[:, 2] = torch.clamp(
        trans_shift[:, 2],
        min=bounds_z_min - left_action_gripper_trans[:, 2],
        max=bounds_z_max - left_action_gripper_trans[:, 2],
    )
    # trans_shift = torch.tensor([[0.1, 0.1, 0.1]])
    trans_shift_4x4 = identity_4x4.detach().clone()
    trans_shift_4x4[:, 0:3, 3] = trans_shift
    ...
    if ver == 1:
        # rotate then translate the 4x4 keyframe action
        left_perturbed_action_gripper_4x4 = torch.bmm(left_action_gripper_4x4, rot_shift_4x4)

        right_perturbed_action_gripper_4x4 = torch.bmm(right_action_gripper_4x4, rot_shift_4x4)
    elif ver == 2:
        right_perturbed_action_gripper_4x4 = identity_4x4.detach().clone()
        right_perturbed_action_gripper_4x4[:, 0:3, 3] = right_action_gripper_4x4[:, 0:3, 3]
        right_perturbed_action_gripper_4x4[:, :3, :3] = torch.bmm(
            rot_shift_4x4.transpose(1, 2)[:, :3, :3], right_action_gripper_4x4[:, :3, :3]
        )

        left_perturbed_action_gripper_4x4 = torch.bmm(
            rot_shift_4x4.transpose(1, 2), left_action_gripper_4x4
        )
    else:
        assert False
    ...
    if ver == 1:
        ...
        left_perturbed_action_gripper_4x4[:, 0:3, 3] += right_action_gripper_4x4[:, 0:3, 3] + trans_shift
    # convert transformation matrix to translation + quaternion
    right_perturbed_action_trans = right_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
    ...
    left_perturbed_action_trans = left_perturbed_action_gripper_4x4[:, 0:3, 3].cpu().numpy()
    ...
    # apply perturbation to pointclouds
    # takes care for not moving the point out of the image
    pcd = perturb_se3(pcd, trans_shift_4x4, rot_shift_4x4, right_action_gripper_4x4, bounds)

    return right_perturbed_action_trans, right_perturbed_action_quat_xyzw, left_perturbed_action_trans, left_perturbed_action_quat_xyzw, pcd

# action_trans_con, action_rot, pc = apply_se3_aug_con(
#                     pcd=None,
#                     action_gripper_pose=torch.tensor([[0.1, 0.1, 0.1,9.88996983e-01, -1.47932634e-01, -9.11104085e-04,1.05259045e-04]]),
#                     bounds=torch.tensor([-0.3, -0.5, 0.6, 0.7, 0.5, 1.6]),
#                     trans_aug_range=torch.tensor([0.1250, 0.1250, 0.1250]),
...
def apply_se3_aug_con_relative2(
    pcd,
    right_action_gripper_pose,
    left_action_gripper_pose,
    bounds,
    ...
    tran_rot_shift_4x4 = identity_4x4.detach().clone()
    tran_rot_shift_4x4[:, :3, :3] = rot_shift_3x3
    tran_rot_shift_4x4[:, 3, :3] = trans_shift
    ...
    if ver == 1:
        ...
        right_perturbed_action_gripper_4x4 = torch.bmm(
            tran_rot_shift_4x4.transpose(1, 2), right_action_gripper_4x4
        )

        left_perturbed_action_gripper_4x4 = torch.bmm(
            tran_rot_shift_4x4.transpose(1, 2), left_action_gripper_4x4
        )
    else:
        assert False
    ...
    # apply perturbation to pointclouds
    # takes care for not moving the point out of the image
    pcd = perturb_se32(pcd, tran_rot_shift_4x4, bounds)
